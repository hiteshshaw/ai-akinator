...
function bestAttribute(players, weights, usedKeys) {
  ...
  for (const a of avail) {
    ...
    let gain = calcEntropy(weights)
      - (pY > 0 ? pY * calcEntropy(yW) : 0)
      - (pN > 0 ? pN * calcEntropy(nW) : 0);

    // Group items with identical or nearly identical highest gain
    if (gain > bestGain + 0.0001) { 
        bestGain = gain; 
        bestGains = [a]; 
    } else if (Math.abs(gain - bestGain) <= 0.0001) {
        bestGains.push(a);
    }
  }
  
  if (bestGain <= 0.0001) {
    return null; // Entropy cannot be meaningfully reduced anymore
  }
  
  return bestGains.length > 0 ? bestGains[Math.floor(Math.random() * bestGains.length)] : avail[0];
}
